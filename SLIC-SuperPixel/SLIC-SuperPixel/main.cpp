                               ...
                               const int& K ) //the number of superpixels desired by the user
{
    ...
    for (int j = 0; j < height; j++)
    {
        for (int k = 0; k < width; k++)
        {
            if (0 > nlabels[oindex])
            {
                //cada novo label começa sendo o label antigo.
				nlabels[oindex] = labels[oindex];
                //--------------------
                // Start a new segment
                //--------------------
                xvec[0] = k;
...
void SuperPixels( Image& rgb, int k, double M )
{
	...
	//FEITO: Converte a imagem de volta.
	rgb = convertImageFromLAB2RGB(pixie);
